...
from rag_pipeline import run_rag_pipeline_sync, RAGResult, SourceDoc
...
def _run_non_stream_completion(user_message: str, use_rag_prompt: bool = False) -> str:
    """Run a non-streaming completion with automatic model fallback via LangChain."""
    ...
    for model in model_candidates:
        try:
            logger.info("Trying LangChain model: %s", model)
...
def _run_stream_completion(user_message: str):
    """Run a streaming completion with automatic model fallback via LangChain."""
    last_error: Optional[Exception] = None

    for model in _get_model_candidates():
        try:
            logger.info("Trying LangChain streaming model: %s", model)
            chain = _get_chain(model)
            # chain.stream() yields string chunks directly (StrOutputParser applied)
...
def generate_blueprint(problem_statement: str, context: Optional[str] = None) -> ProjectBlueprint:
    ...
    try:
        ...
        # ── Step 3: merge RAG sources into learning_references ────────────
        if rag_result.sources:
            ...
            if new_refs_data:
                ...
                blueprint.learning_references = list(blueprint.learning_references or []) + new_refs
                logger.info("Appended %d RAG source links to learning_references", len(new_refs))
...
def generate_ui_preview(project_name: str, context: Optional[str] = None) -> str:
    ...
    for model in _get_model_candidates():
        try:
            logger.info("Generating UI preview with model: %s", model)
            llm = ChatGroq(
                api_key=settings.groq_api_key,
                model_name=model,
                temperature=0.4,
            ...
            logger.info("UI preview generated successfully with model: %s", model)
            return cleaned

        except Exception as e:
            last_error = e
            logger.warning("UI preview model %s failed: %s", model, str(e))
            if not _is_retryable_model_error(e):
                raise
...
def generate_runtime_flow(project_name: str, context: Optional[str] = None) -> list:
    ...
    for model in _get_model_candidates():
        try:
            logger.info("Generating runtime flow with model: %s", model)
            llm = ChatGroq(
                api_key=settings.groq_api_key,
                model_name=model,
                temperature=0.2,
            ...
            # Sanitise: ensure required keys, clamp lane values
            ...
            logger.info("Runtime flow generated: %d steps with model %s", len(steps), model)
            return steps

        except Exception as e:
            last_error = e
            logger.warning("Runtime flow model %s failed: %s", model, str(e))
            if not _is_retryable_model_error(e):
                raise
...
#     "arxiv":            ("Research Paper",  "Advanced"),
#     "semantic_scholar": ("Research Paper",  "Advanced"),
#     "crossref":         ("Documentation",   "Intermediate"),
#     "core":             ("Research Paper",  "Advanced"),
#     "tavily":           ("Guide",           "Beginner"),
#     "github":           ("Guide",           "Intermediate"),
# }
...
# def _sources_to_learning_references(sources: List[SourceDoc]) -> List[dict]:
#     """Convert RAG SourceDoc list → learning_references dicts for blueprint injection."""
#     refs = []
#     seen = set()
#     for doc in sources:
#         if not doc.url or doc.url in seen:
#             continue
#         seen.add(doc.url)
#         ref_type, difficulty = SOURCE_TYPE_MAP.get(doc.source, ("Guide", "Intermediate"))
#         refs.append({
#             "title":      doc.title or doc.url,
#             "url":        doc.url,
#             "type":       ref_type,
...
#     if rag_context:
#         parts.append(
#             f"\n--- RESEARCH CONTEXT (from Semantic Scholar, arXiv, CrossRef, CORE, Tavily, GitHub) ---\n"
#             f"{rag_context}\n--- END RESEARCH CONTEXT ---"
#         )
#     parts.append("Audience: Beginner developer who needs step-by-step implementation clarity.")
#     return "\n".join(parts)
...
# RAG_SYSTEM_PROMPT = """You are an expert AI system architect with access to real research papers, documentation, and open-source repositories. Output ONLY valid JSON (no markdown) matching this schema exactly:
# {{
#   "project_name": string,
#   "description": string,
#   "problem_statement": string,
...
# }}
# Rules:
# - The RESEARCH CONTEXT in the user message contains real excerpts from academic papers, documentation, and GitHub repos — use them to make the blueprint more accurate and grounded.
# - Cite insights from the research context when shaping tech stack choices, solution approaches, and real-world examples.
# - Target audience is beginners. Use simple language and practical explanations.
# - Keep medium detail level: complete but concise.
# - Workflow must be end-to-end with 7-10 steps, step_number strictly sequential from 1.
# - Each workflow step must include 2+ components_involved, 3-4 key_actions, and a clear phase outcome.
# - Tech stack should be practical (8-12 items) and include frontend, backend, database, infrastructure, APIs, testing, CI/CD, and security.
# - For every tech_stack item, include languages, frameworks, and modules arrays with concrete names.
# - timeline should represent realistic phases from setup to launch.
# - next_steps must be actionable and beginner-friendly.
...
# # -------------------------------------------------------------------------------------
# # Internal completion runners (same names as original)
# # -------------------------------------------------------------------------------------

# def _run_non_stream_completion(user_message: str) -> str:
#     """Run a non-streaming completion with automatic model fallback via LangChain."""
#     last_error: Optional[Exception] = None
...
# def generate_blueprint(problem_statement: str, context: Optional[str] = None) -> ProjectBlueprint:
#     """Generate a project blueprint using LangChain + Groq."""
#     try:
#         user_message = (
...
#         response_text = _run_non_stream_completion(user_message)
#         if not response_text:
#             raise ValueError("Model returned an empty response")
...
#         except ValueError:
#             # One auto-retry with stricter formatting hint
#             retry_message = (
#                 user_message
#                 + "\n\nIMPORTANT: Return ONLY valid JSON object. "
#                   "No markdown, no explanations, no trailing text. "
#                   "If content is long, shorten sentences but keep schema complete."
#             )
#             retry_text = _run_non_stream_completion(retry_message)
#             if not retry_text:
#                 raise ValueError("Model returned an empty response on retry")
...
#     try:
#         # chain.stream() yields plain strings — no .choices[0].delta needed
#         stream = _run_stream_completion(user_message)
#         for chunk in stream:
#             if chunk:
#                 yield chunk
...
#     )
# --------------------------------------------------------------
# Normal API model (non-LangChain) 
